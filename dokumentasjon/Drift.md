...
`select * from krav where saksnummer_nav = '<saksnummer>'`
Hvis det har skjedd noe feil kan du finne feilmelding i Feilmelding:
`select * from feilmelding where saksnummer_nav = '<saksnummer>'`
...
`update krav set status = 'KRAV_IKKE_SENDT' where saksnummer_nav = '<saksnummer>'`

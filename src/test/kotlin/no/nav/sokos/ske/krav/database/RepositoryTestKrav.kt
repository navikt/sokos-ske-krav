...
import no.nav.sokos.ske.krav.util.DBUtils.useSession
...
internal class RepositoryTestKrav :
    FunSpec({
        ...
        DBListener.migrate("SQLscript/KravForRepositoryBehaviourTestScript.sql")
        ...
        test("getKravTableIdFromCorrelationId skal returnere krav_id basert på corr_id") {
            DBListener.dataSource.useSession { session ->
        ...
        test("updateStatusForAvstemtKravToReported skal sette rapporter til false på krav med angitt kravid") {
            ...
            val feilmelding1 = FeilmeldingRepository.getFeilmeldingForKravId(DBListener.session, firstKrav.kravId)
            val feilmelding2 = FeilmeldingRepository.getFeilmeldingForKravId(DBListener.session, lastKrav.kravId)

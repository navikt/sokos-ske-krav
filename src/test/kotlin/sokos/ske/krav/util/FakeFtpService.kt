		...
		val jsch = mockk<JSch>(relaxUnitFun = true, relaxed = true) {
			every { getSession(any<String>(), any<String>(), any<Int>()) } returns mockk<Session>(relaxed = true) {
				every { openChannel("sftp") } returns mockk<ChannelSftp>(relaxed = true) {
					...
					} answers { client.retrieveFile(firstArg<String>(), secondArg<ByteArrayOutputStream>()) }
					every { put(any<InputStream>(), any<String>()) } answers {
						fakeFtpServer.fileSystem.add(
							FileEntry(
								secondArg<String>(),
								firstArg<InputStream>().toString()
							)
						)
					}
					every {
					...
					} answers { fakeFtpServer.fileSystem.rename(firstArg<String>(), secondArg<String>()) }
				}

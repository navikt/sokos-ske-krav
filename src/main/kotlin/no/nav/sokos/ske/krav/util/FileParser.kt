...
object FileParser {
    fun parseKontrollLinjeHeader(content: List<String>) = kontrollLinjeHeaderParser(content.first())

    fun parseKontrollLinjeFooter(content: List<String>) = kontrollLinjeFooterParser(content.last())

    fun parseKravLinjer(content: List<String>) = content.subList(1, content.lastIndex).map { kravLinjeParser(it) }
